...
public class IssueController {
    ...
    public static ResultSet getIssueInfo(String year,String month) throws ClassNotFoundException, SQLException {
        ...
        String query = "SELECT  * FROM issue INNER JOIN issuedetail ON issue.IssueID = issuedetail.IssueID INNER JOIN bloodpacket ON issuedetail.PacketID = bloodpacket.PacketID WHERE issue.Date> '" + year+ "-" + Integer.toString(monthNo) + "-01' AND issue.Date < '" + year+ "-" + Integer.toString(monthNo) + "-31'";
    ...
    public static ResultSet getCrossMatchedDetails() throws ClassNotFoundException, SQLException {

        String query = "SELECT  * FROM crossmatchdetail";
        Connection connection = DBConnection.getConnectionToDB();
    ...
    public static void updateIssue(String requestNo, String packetID, String issueID,String employeeid,String date, String time) throws ClassNotFoundException, SQLException {
        ...
        query = "UPDATE bbms.sampledetail SET `IssueID` = '"+issueID+"' WHERE `RequestNo` = '"+requestNo+"'";
        connection = DBConnection.getConnectionToDB();
        DBHandler.setData(connection, query);      
    }
    ...
    public static ResultSet getRequesteeInfo(String year, String month) throws ClassNotFoundException, SQLException{
        ...
        String query = "SELECT  * FROM issue INNER JOIN sampledetail ON issue.sampleID = sampledetail.requestNo WHERE issue.Date> '" + year+ "-" + Integer.toString(monthNo) + "-01' AND issue.Date < '" + year+ "-" + Integer.toString(monthNo) + "-31'";
        Connection connection = DBConnection.getConnectionToDB();
